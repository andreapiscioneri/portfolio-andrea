...
<template>
  <div>
    <section class="relative overflow-hidden pt-40 md:pt-48 lg:pt-56 pb-14 md:pb-20">
      ...
      <div class="pointer-events-none absolute inset-0 bg-[radial-gradient(circle_at_20%_18%,rgba(57,255,20,0.18),transparent_50%)]" />
    ...
    <section class="container-x pt-16 md:pt-20">
      <div class="flex flex-wrap items-center gap-2">
        <button
          ...
          :class="selected === '__all' ? 'border-accent text-accent' : 'border-white/15 text-white/60 hover:text-accent hover:border-accent/50'"
          data-cursor="link"
        ...
        <button
          ...
          type="button"
          class="rounded-full border px-4 py-2 text-xs uppercase tracking-[0.16em] transition-colors duration-300"
          :class="selected === c ? 'border-accent text-accent' : 'border-white/15 text-white/60 hover:text-accent hover:border-accent/50'"
          data-cursor="link"
          @click="selected = c"
        >
          {{ c }}
